...
export function ExpenseDailyChart({ data }: ExpenseDailyChartProps) {
  const { t } = useTranslation('reports')

  if (!data || data.length === 0) {
    return (
      <div className="chart-empty">
        <p>{t('expense.charts.emptyExpenses')}</p>
      </div>
    )
  }
  ...
  return (
    <div className="chart-container">
      <h3>{t('expense.charts.dailyTotals')}</h3>
      <ResponsiveContainer width="100%" height={300}>
        <LineChart data={chartData} margin={{ top: 20, right: 30, left: 20, bottom: 20 }}>
          ...
          <Line 
            ...
            stroke="#dc3545" 
            strokeWidth={2}
            name={t('expense.charts.legendDailyTotal')}
            dot={{ r: 4 }}
          />
        </LineChart>
      </ResponsiveContainer>
